...
import { Star, MapPin, Phone, Mail, Globe, Filter, Search, ArrowRight, Hotel as HotelIcon, Award, Users, DollarSign } from 'lucide-react';
...
export default function AllHotels() {
  ...
  const filteredHotels = useMemo(() => {
    ...
    // Country filter
    if (selectedCountry) {
      result = result.filter(hotel => hotel.countryId === selectedCountry);
    }
    ...
    return result;
  }, [hotels, selectedCountry, searchQuery, filterStars, filterPrice, sortBy, language]);
        ...
        <div className="container mx-auto px-4 relative z-30 text-center">
          <div className="inline-flex items-center gap-2 mb-4 px-4 py-2 bg-white/10 backdrop-blur-md rounded-full border border-white/20">
            ...
            <span className="text-sm font-medium">
              {language === 'ar' ? 'حجز الفنادق' : language === 'fr' ? 'Réservation d\'hôtels' : 'Hotel Booking'}
            </span>
          </div>
          <h1 className="text-5xl md:text-6xl font-extrabold mb-4">
            {language === 'ar' ? 'احجز فندقك المثالي' : language === 'fr' ? 'Réservez votre hôtel idéal' : 'Book Your Perfect Hotel'}
          </h1>
          <p className="text-xl text-white/90 max-w-2xl mx-auto">
            {language === 'ar' 
              ? 'اكتشف أفضل الفنادق والمنتجعات في جميع أنحاء العالم'
              : language === 'fr'
              ? 'Découvrez les meilleurs hôtels et resorts du monde entier'
              : 'Discover the best hotels and resorts around the world'}
          </p>
        </div>
      </section>
            ...
            <>
              <div className="mb-6 flex items-center justify-between">
                <h2 className="text-2xl font-bold text-gray-900">
                  {language === 'ar' 
                    ? `تم العثور على ${filteredHotels.length} فندق${filteredHotels.length > 1 ? 'ات' : ''}`
                    : language === 'fr'
