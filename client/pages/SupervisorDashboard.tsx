...
const SupervisorDashboard: React.FC = () => {
  ...
  // Check authentication and load data from server
  useEffect(() => {
    ...
    (async () => {
      // تحميل الدول والمكاتب من السيرفر حتى تظهر التعديلات على كل الأجهزة
      ...
      loadStatistics();
      loadActivities();
      await loadOffersForCountry();
    })();
  }, [supervisor, navigate]);

  useEffect(() => {
    if (activeTab === 'offers') {
      loadOffersForCountry();
    }
  ...
  const loadOffersForCountry = async () => {
    if (!supervisor) return;
    await dataManager.ensureMinimumOffersForCountries(10, [supervisor.countryId]);
    const offersForCountry = dataManager.getOffersByCountry(supervisor.countryId);
    setOffers(offersForCountry);
  };
  ...
  const content = {
    ar: {
      ...
      edit: 'تعديل',
      view: 'عرض',
      addOffer: 'إضافة عرض سياحي',
      editOffer: 'تعديل العرض',
      noOffers: 'لا توجد عروض سياحية',
      price: 'السعر',
      duration: 'المدة (أيام)',
      featured: 'عرض مميز',
      country: 'الدولة',
    ...
    en: {
      ...
      edit: 'Edit',
      view: 'View',
      addOffer: 'Add Tour Offer',
      editOffer: 'Edit Offer',
      noOffers: 'No tour offers yet',
      price: 'Price',
    ...
    fr: {
      ...
      noOffers: 'Aucune offre touristique',
      price: 'Prix',
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  {supervisor.permissions.canAddOffers && (
                    <Button
                      className="w-full justify-start"
                      variant="outline"
                      onClick={() => navigate('/supervisor/offers/add')}
                    >
                      <Plus className="w-4 h-4 mr-2" />
                      {text.addOffer}
                    </Button>
                ...
                {country.cities && country.cities.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {country.cities.map((city, index) => {
                      ...
                      const cityId = city.id || `city_${supervisor.countryId}_${index}`;
                      
                      return (
                        <Card key={cityId || index}>
                          <CardContent className="p-4">
                            ...
                            <div className="flex space-x-2">
                              <Button 
                                ...
                                variant="outline"
                                onClick={() => {
                                  // Navigate to city detail page if available
                                  navigate(`/offices/${supervisor.countryId}/city/${cityId}`);
                                }}
                              >
                              ...
                              {supervisor.permissions.canEditCities && (
                                <Button 
                                  ...
                                  variant="outline"
                                  onClick={() => {
                                    navigate(`/supervisor/cities/edit/${cityId}`);
                                  }}
                                >
            ...
            <Card>
              <CardHeader>
                <div className="flex justify-between items-center">
                  ...
                  {supervisor.permissions.canAddOffers && (
                    <Button onClick={() => navigate('/supervisor/offers/add')}>
                      <Plus className="w-4 h-4 mr-2" />
                      {text.addOffer}
                    </Button>
                  )}
                      ...
                      <Card key={offer.id}>
                        <CardContent className="p-4 space-y-3">
                          <div className="flex items-start justify-between">
                            <div>
                              <h3 className="font-semibold text-gray-900">{getLocalized(offer.title)}</h3>
                              <p className="text-sm text-gray-600 line-clamp-2">
                                {getLocalized(offer.description)}
                              </p>
                            </div>
                            {offer.isFeatured && (
                          ...
                          <div className="flex items-center justify-between text-sm text-gray-700">
                            ...
                            <span>
                              {text.duration}: {offer.durationDays}
                            </span>
                          </div>
                          <div className="flex items-center justify-between">
                            <Badge variant={offer.isActive ? "default" : "secondary"}>
                              {offer.isActive ? text.active : text.inactive}
                            </Badge>
                            {supervisor.permissions.canEditOffers && (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => navigate(`/supervisor/offers/edit/${offer.id}`)}
                              >
